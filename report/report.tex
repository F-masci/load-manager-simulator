...
La modifica è implementata nella classe \texttt{ThresholdSpikeServerRoutingStrategy}. La classe mantiene una mappa \texttt{spikeModeMap}, indicizzata per identificativo del Web Server, che conserva lo stato corrente del routing. Il parametro \texttt{siLow} rappresenta la soglia inferiore; quando non è specificato, assume lo stesso valore di $SI_{max}$ e il comportamento coincide con il modello base.

\begin{lstlisting}[language=Java]
int currentSiLow = (siLow == -1) ? siMax : siLow;
boolean currentlyInSpike =
    spikeModeMap.getOrDefault(targetServer.getId(), false);
