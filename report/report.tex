...
Per ragioni di efficienza, l'implementazione software rigetta l'approccio a due passaggi in favore di una formulazione algebricamente alternativa di tipo \textit{one-pass}, come descritto in \cite{leemis-park-book}. L'autocovarianza per il lag $j$ viene determinata isolando la sommatoria dei co-prodotti sfasati direttamente dalla media quadratica:
\begin{equation}
    c_j = \left( \frac{1}{n-j}\sum_{i=1}^{n-j}x_i x_{i+j} \right) - \bar{x}^2
\end{equation}
...
\end{lstlisting}
Le repliche non ripartono quindi tutte da \texttt{123456789}: ciascuna prosegue la generazione a partire dall'ultimo stato prodotto dalla precedente, utilizzando porzioni successive della sequenza pseudo-casuale. Questa concatenazione evita di ripetere la stessa traiettoria in ogni run indipendente e, poiché il primo seed e la durata delle repliche sono deterministici, conserva la riproducibilità dell'intera campagna.
...
\begin{figure}[H]
    ...
    \caption{Modulazione istantanea del fattore di capacità elaborativa in funzione del numero di job residenti nella coda dello Spike Server per lo Scaling Verticale.}
...
L'analisi transitoria è stata condotta tramite la classe \texttt{TransientAnalysisObjective}. A differenza delle campagne steady-state, qui il sistema viene osservato a partire da uno stato iniziale vuoto, usando repliche indipendenti e salvando l'evoluzione temporale del tempo medio di risposta. Sono stati considerati sei scenari sulla sola architettura completa: due a basso carico, due a carico medio e due ad alto carico, variando sia $\lambda$ sia il coefficiente di variazione degli interarrivi. Ogni scenario è stato eseguito con $10$ repliche indipendenti (i cui seed sono riportati nei grafici); gli scenari low e medium sono stati simulati fino a circa $1008000\text{ s}$ ($=280\text{h}$), mentre gli scenari high fino a $504000\text{ s}$ ($=140\text{h}$), con scala temporale dei grafici convertita in ore per renderli leggibili.
...
La Tabella~\ref{tab:burstiness_results} evidenzia un risultato importante: l'incremento della burstiness non porta il sistema fuori controllo. Al contrario, a parità di $\lambda$, l'aumento di $C_V$ tende a far crescere l'uso delle risorse elastiche, ma mantiene il tempo medio di risposta in una regione ampiamente accettabile rispetto allo SLA. Per $\lambda=11$, ad esempio, $R_0$ passa da $3.9993\text{ s}$ con $C_V=1.0$ a $3.6098\text{ s}$ con $C_V=12.0$; questo non significa che un traffico più bursty sia intrinsecamente più semplice, ma che il meccanismo di protezione intercetta più frequentemente i picchi e sposta lavoro verso lo Spike Server e verso configurazioni con più Web Server attivi. L'intera griglia rimane sotto SLA: il massimo osservato è $4.2853\text{ s}$ per $C_V=4$ e $\lambda=14$, con Half-Width pari a $0.0199\text{ s}$.

Le figure ~\ref{fig:burstiness_rt_jis}--\ref{fig:burstiness_spike} confermano la natura adattiva dell'architettura. Il throughput segue correttamente il tasso di arrivo, segnalando assenza di perdita strutturale di capacità; il numero di job nel sistema cresce con $\lambda$, ma resta compatibile con tempi medi di risposta sotto soglia; le azioni di scaling aumentano nelle regioni più intense, ma non producono divergenze. La presenza delle barre di deviazione standard e del contorno dell'intervallo di confidenza permette inoltre di distinguere il rumore statistico dalle tendenze effettive: le curve restano separate soprattutto per le metriche operative, mentre il tempo di risposta mostra intervalli contenuti rispetto al margine disponibile sullo SLA.
...
Un secondo controllo riguarda la probabilità operazionale di dirottamento. Poiché ciascun batch contiene $4096$ completamenti e il numero medio di job deviati è $\hat{D}=197.0063$, la frazione osservata di traffico instradata allo Spike Server è:
\begin{equation}
