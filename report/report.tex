...
% TODO: verificarla bene
% A regime, la probabilità $q_i$ corrisponde al valore atteso della funzione indicatrice condizionato agli istanti di arrivo dei job candidati, ovvero:
% \begin{equation}
% q_i = \mathbb{E}[d_i(t^-)] = P(SI_i(t^-) \ge SI_{max}),
% \end{equation}
% dove $t^-$ indica l'istante immediatamente precedente un arrivo. È importante sottolineare che, a causa della natura iperesponenziale del processo degli % arrivi (caratterizzato da un coefficiente di variazione $C_V > 1$), non si applica la proprietà PASTA. Pertanto, la probabilità di dirottamento $q_i$ % differisce dalla mera media temporale dell'indicatore, riflettendo l'effetto di "bunching" tipico dei flussi di traffico a varianza elevata.
...
La libreria \texttt{lib.rng} deriva dalle classi \texttt{Rngs}, \texttt{Rvgs} e \texttt{Rvms} di Park, Miller e collaboratori, adottate nei testi di simulazione per la generazione di numeri pseudo-casuali e variate aleatorie \cite{leemis-park-book}. Il generatore di base è un Lehmer multiplicative congruential generator:
\begin{equation}
