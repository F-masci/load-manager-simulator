...
\newcommand{\sectionframe}[3]{%
  ...
  \begin{frame}[plain,noframenumbering]
    ...
    \vspace{0.28cm}
    {\color{ink}\bfseries\Huge #2}\par
    \vspace{0.22cm}
    {\color{teal}\rule{1.6cm}{2pt}}\par
    \vspace{0.55cm}
    ...
    \begin{tikzpicture}[remember picture,overlay]
      \fill[teal!8] ([xshift=-3.8cm,yshift=-1.2cm]current page.north east)
        circle (2.1cm);
      \fill[coral!10] ([xshift=-0.8cm,yshift=0.4cm]current page.south east)
        circle (2.8cm);
      \fill[gold!16] ([xshift=-2.1cm,yshift=1.2cm]current page.south east)
        circle (1.0cm);
    \end{tikzpicture}
  \end{frame}
}
...
\author{Francesco Masci, 0365258}
...
\newcommand{\frameConc}{%
  \stepcounter{concnum}
  Concettuale \Roman{concnum}
}

\newcommand{\frameSpec}{%
  \stepcounter{specnum}
  Specifiche \Roman{specnum}
}

\newcommand{\frameComp}{%
  \stepcounter{compnum}
  Computazionale \Roman{compnum}
}
...
\begin{frame}[plain]
  ...
  \begin{minipage}{0.74\textwidth}
    ...
    \vspace{0.2cm}
    {\scriptsize\color{white!58}\insertinstitute}\par
    {\scriptsize\color{white!50}Corso di Laurea Magistrale in Ingegneria Informatica}\par
    {\scriptsize\color{white!50}Performance Modeling of Computer Systems and Networks}\par
    \vspace{0.24cm}
    {\scriptsize\color{white!58}\insertdate}
  \end{minipage}
\end{frame}
}
...
\sectionframe{01}{Sistema e obiettivi}{Architettura, carico bursty e domanda di ricerca.}
...
% 4 ------------------------------------------------------------------------
\subsection{Obiettivi sperimentali}
...
% --------------------------------------------------------------------------
\sectionframe{02}{Modellazione}{Rete di code, stato e specifiche operative.}

\subsection{Modello concettuale}
\begin{frame}{\frameConc: una rete di code gerarchica}
  \centering
...
% --------------------------------------------------------------------------
\begin{frame}{\frameConc: lo stato del sistema}
  \[
    \boxed{
      S(t)=\left\{
...
% --------------------------------------------------------------------------
% \begin{frame}{\frameConc: Tre decisioni, tre scale di reazione}
%   \begin{columns}[T,onlytextwidth]
%     \column{.305\textwidth}
%     \begin{block}{Routing}
%       \centering
...
\begin{frame}{\frameSpec: workload e disciplina di servizio}
  \small
  \begin{columns}[T,onlytextwidth]
    \column{.48\textwidth}
    ...
    \begin{itemize}
      ...
      \item \(\mu_{web}=\mu_{base}=4.0\) job/s
      \item anche il servizio è iperesponenziale: \(C_V^S=4.0\).
    \end{itemize}
    \vspace{0.04cm}

    \tagbox{coral}{DISCIPLINA PROCESSOR SHARING}
    \[
...
% --------------------------------------------------------------------------
% \begin{frame}{\frameSpec: probabilità di routing}
%   \small
%   \begin{columns}[T,onlytextwidth]
%     \column{.48\textwidth}
...
% --------------------------------------------------------------------------
\begin{frame}{\frameSpec: parametri fissati del controllo}
  \begin{columns}[T,onlytextwidth]
    \column{.54\textwidth}
...
% --------------------------------------------------------------------------
\sectionframe{03}{Implementazione}{Simulatore next-event e analisi statistica.}

\subsection{Componenti del simulatore}
\begin{frame}[fragile=singleslide]{\frameComp: oggetti di dominio e Processor Sharing}
  \begin{columns}[T,onlytextwidth]
    \column{.54\textwidth}
...
% --------------------------------------------------------------------------
\begin{frame}[fragile=singleslide]{\frameComp: motore next-event}
  \begin{columns}[T,onlytextwidth]
    \column{.58\textwidth}
...
% --------------------------------------------------------------------------
% \begin{frame}[fragile=singleslide]{\frameComp: routing e autoscaler componibili}
%   \begin{columns}[T,onlytextwidth]
%     \column{.58\textwidth}
%     \resizebox{\linewidth}{!}{%
%       \begin{tikzpicture}[
...
\begin{frame}[fragile=singleslide]{\frameComp: Batch Means e repliche indipendenti}
  \begin{columns}[T,onlytextwidth]
    \column{.31\textwidth}
    \begin{block}{Builder}
...
% --------------------------------------------------------------------------
\sectionframe{04}{Verifica e validazione}{Tracce controllate e confronto esterno.}
...
% --------------------------------------------------------------------------
\subsection{Validazione esterna}
...
% 8 ------------------------------------------------------------------------
\sectionframe{05}{Risultati}{Dimensionamento, costo e robustezza.}
...
\begin{frame}{Obiettivo 1A: scegliere la politica di routing}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Simulazione stazionaria con Batch Means; \(4\) Web Server, \(H_2/H_2\), \(\lambda=5\), \(C_V=4\).}
    \column{.33\textwidth}
...
\begin{frame}{Obiettivo 1B: calibrare la soglia di deviazione}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Simulazione stazionaria con Batch Means; un Web Server, Spike Server attivo, scaling disabilitato.}
    \column{.31\textwidth}
    \tagbox{teal}{SCELTA}\par\vspace{0.12cm}
...
\begin{frame}{Obiettivo 1C: dimensionare lo scaling verticale}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Simulazione stazionaria con Batch Means; \(SI_{\max}=40\), \(\Delta v_s\in[0.5,5.0]\).}
    \column{.31\textwidth}
    \tagbox{teal}{SCELTA}\par\vspace{0.12cm}
...
\begin{frame}{Obiettivo 1D: tracciare la frontiera orizzontale}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Convergenza a orizzonte finito e stima steady-state con Batch Means; SLA \(R_0\le6\,s\).}
    \column{.30\textwidth}
...
%   \begin{center}
%     \begin{tikzpicture}[node distance=.48cm,every node/.style={font=\small}]
...
\begin{frame}{Costo: quando il controller diventa troppo lento}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Simulazione stazionaria con Batch Means; costo e \(R_0\) con cooldown \(100\,s\).}
    \column{.35\textwidth}
    \tagbox{cyan}{FUNZIONE DI COSTO}\par\vspace{-0.55cm}
    \[
      C_{tot}=\bar{N}C_{WS}
      +\bar{U}_{spike}\bar{S}_{spike}C_{CPU}
    \]
    {\scriptsize \(C_{WS}=2.5,\quad C_{CPU}=1.0\)}
    \vspace{0.24cm}
...
% --------------------------------------------------------------------------
\subsection{Costo del controllo}
\begin{frame}{Costo: il cooldown conta}
  \begin{columns}[T,onlytextwidth]
    \column{.62\textwidth}
    \includegraphics[width=\linewidth,height=.69\textheight,keepaspectratio]{../data/objective/cost/cost_analysis_cooldown_10s.png}
    \source{Simulazione stazionaria con Batch Means: \(2048\) batch da \(4096\) job, warm-up \(5\cdot10^5\) job.}
    \column{.35\textwidth}
    \vspace{0.44cm}
...
\begin{frame}{Analisi del transitorio: il sistema si assesta}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Simulazione a orizzonte finito con repliche indipendenti; \(\lambda=5\), \(C_V=4\), sistema vuoto.}
    \column{.34\textwidth}
...
% 12 -----------------------------------------------------------------------
\subsection{Robustezza alla burstiness}
\begin{frame}[shrink=3]{Burstiness: la gerarchia redistribuisce il carico}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Simulazione stazionaria con Batch Means: \(1024\) batch da \(4096\) job, warm-up \(5\cdot10^5\) job.}
    \column{.35\textwidth}
    \begin{center}
...
% 13 -----------------------------------------------------------------------
\sectionframe{06}{Miglioramento}{Banda di stabilità e sintesi finale.}

\subsection{Meccanismo a bande}
...
%     \source{Simulazione trace-driven deterministica; \(SI_{\max}=5\), \(SI_{\mathrm{low}}=2\), 21 arrivi noti.}
%     \column{.29\textwidth}
...
\begin{frame}{Sistema modificato: meno latenza, più uso dello spike}
  \begin{columns}[T,onlytextwidth]
    ...
    \source{Simulazione stazionaria con Batch Means; banda \(B=10\), utilizzazioni e \(R_0\).}
    \column{.39\textwidth}
    \begin{center}
    \renewcommand{\arraystretch}{1.25}
    \begin{tabular}{@{}lrr@{}}
