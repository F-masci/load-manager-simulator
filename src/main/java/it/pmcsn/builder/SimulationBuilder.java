package it.pmcsn.builder;

import it.pmcsn.configs.ApplicationConfig;
import it.pmcsn.controller.SimulationController;
import it.pmcsn.controller.SimulationController.RoutingPolicy;
import it.pmcsn.model.load.LoadManager;
import it.pmcsn.event.source.EventSource;
import it.pmcsn.event.source.ExponentialEventSource;
import it.pmcsn.event.source.HyperexponentialEventSource;
import it.pmcsn.event.source.TraceEventSource;
...
import it.pmcsn.model.load.scaler.horizontal.MovingWindowHorizontalScaler;
import it.pmcsn.model.load.scaler.vertical.VerticalScaler;
import it.pmcsn.model.load.scaler.vertical.UtilizationThresholdVerticalScaler;
import it.pmcsn.model.server.*;
import it.pmcsn.model.load.routing.Router;
import it.pmcsn.model.load.routing.RoundRobinRouter;
import it.pmcsn.model.load.routing.LeastLoadedRouter;
...
public class SimulationBuilder {
    ...
    private it.pmcsn.configs.WorkloadType workloadType = it.pmcsn.configs.WorkloadType.DISTRIBUTION;
    ...
    public SimulationBuilder config(ApplicationConfig config) {
        ...
        try {
            if (workloadType == it.pmcsn.configs.WorkloadType.TRACE && config.tracePath() != null) {
                this.traceFile(config.tracePath());
            } else if (workloadType == it.pmcsn.configs.WorkloadType.HYPEREXPONENTIAL) {
    ...
    public SimulationController build() {
        if (eventSource == null) {
            if (workloadType == it.pmcsn.configs.WorkloadType.HYPEREXPONENTIAL) {
